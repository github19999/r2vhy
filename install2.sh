...
generate_reality_keypair() {
    sing-box generate reality-keypair 2>/dev/null | awk '/PrivateKey:/ {print $2}' > /tmp/private_key
    sing-box generate reality-keypair 2>/dev/null | awk '/PublicKey:/ {print $2}' > /tmp/public_key
    PRIVATE_KEY=$(cat /tmp/private_key)
    PUBLIC_KEY=$(cat /tmp/public_key)
    rm -f /tmp/private_key /tmp/public_key
}
